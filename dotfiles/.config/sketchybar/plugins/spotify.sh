...
update_track() {
  ...
  if [[ -z $PLAYER_STATE || $PLAYER_STATE = "Stopped" ]]; then
    ...
    sketchybar --set $NAME icon.drawing=no label.drawing=no icon.padding_right=14 icon.padding_left=14
    # If playing
  elif [ $PLAYER_STATE = "Playing" ]; then
        sketchybar --set $NAME label="${TRACK}  ${ARTIST}" icon.drawing=yes label.drawing=yes icon.padding_right=8
